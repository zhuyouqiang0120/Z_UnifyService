...
<script>
	$(function() {
		...
		var add = function(t,grid) {
			var html = '<form class="form-horizontal" id="form1" role="form" style="padding:10px;">';
				html += '<input id="id" value="" name="id" type="hidden">';
				html += '<div class="form-group"><label for="name" class="col-sm-3 control-label">功能名称:</label><div class="col-sm-9"><input type="text" class="form-control" name="name" id="name"></div></div>';
				html += '<div class="form-group"><label for="isTask" class="col-sm-3 control-label">定时任务:</label><div class="col-sm-9">  是  <input type="radio" checked class="radio-inline" name="isTask" value="1">  否<input type="radio" class="radio-inline" name="isTask" value="0"></div></div>';
				html += '<div class="form-group"><label for="url" class="col-sm-3 control-label">数据地址:</label><div class="col-sm-9"><input type="text" class="form-control" name="url" id="url"></div></div>';
				html += '<div class="form-group"><label for="code" class="col-sm-3 control-label">类型:</label><div class="col-sm-9"><input type="text" class="form-control" name="code" id="code"></div></div>';
				html += '<div class="form-group"><label for="cycle" class="col-sm-3 control-label">时间表达式:</label><div class="col-sm-9"><input type="text" class="form-control" name="cycle" id="cycle"></div></div>';
				html += '<div class="form-group"><label for="menuUrl" class="col-sm-3 control-label">系统地址:</label><div class="col-sm-9"><input type="text" class="form-control" name="menuUrl" id="menuUrl"></div></div></form>';
		
			var addIndex = $.layer({
				type: 1,
				title: [t ? t : "新增信息",'border:1px; background:#61BA7A; color:#fff;'],
				bgcolor: '#eee',
				area: ['590px', '360px'],
				shade: [0.3, '#000'],
				shadeClose: false,
		...
		var setData = function(item) {
			...
			$("input[name='isTask']").removeAttr("checked");
			$("input[name='isTask'][value='"+item.isTask+"']").attr("checked",true);
		}
		/*
		 *   保存数据 id存在则为修改 否则为新增
		 */
